...
export class EventService {
  ...
  constructor(private _connection: Connection) {
    ...
    this._eventCompetitionRepository =
      this._connection.getRepository(EventCompetition);
    this._eventRegistrationRepository =
      this._connection.getRepository(EventRegistration);
  }
  ...
  async updateEventRegistration(
    ...
    if (eventRegistration) {
      eventRegistration.isVerified =
        isVerified === false
          ? false
          : isVerified || eventRegistration.isVerified;
  ...
  async findQuizScore(quizId: number, userId: number) {
    ...
    const totalTimeSpent = compInfo.reduce(
      (previousValue, currentValue) => previousValue + currentValue.timeSpent,
      0,
    );
    const correctAnswers = compInfo.filter(
      (answer) =>
        answer.option &&
        answer.question.options.find(
          (opt) => opt.id === answer.option && opt.isCorrect,
        ),
    );
    const totalScore = correctAnswers.reduce(
      (previousValue, currentValue) =>
        previousValue + currentValue.question.category.score,
      0,
    );
    const wrongAnswers = compInfo.filter(
      (answer) =>
        answer.option &&
        answer.question.options.find(
          (opt) => opt.id !== answer.option && !opt.isCorrect,
  ...
  async update(id: number, updatedEvent: UpdateEventInput) {
    try {
      ...
      event.isRegistrationOpen =
        updatedEvent.isRegistrationOpen || event.isRegistrationOpen;
      ...
      event.image = updatedEvent.image || event.image;
